...
const getTimeDiff = (start, end) => {
    ...
    return (Number(start_h) * 60 + Number(start_m)) - (Number(end_h) * 60 + Number(end_m))
}
...
const makeMap = (arr) => {
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        if(action === "IN"){
            ...
            if(feeMap.has(car_num)){
                const sum = feeMap.get(car_num) + getTimeDiff(time, parkingLot.get(car_num))
    ...
    for(let [car_num, time] of parkingLot){
        if(parkingLot.has(car_num)) {
            feeMap.set(car_num, (feeMap.get(car_num) | 0) + getTimeDiff("23:59", time));
        }
    }
